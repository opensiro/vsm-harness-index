...
2. [`assess-vsm-harness`](https://github.com/opensiro/vsm-harness-skills/tree/main/skills/assess-vsm-harness) for repository evidence and ownership classification;
...
**Assessment.** `assessments/<harness_id>.md` is repository-relative and revision-relative. It stores the review boundary, repository architecture, VSM mappings, evidence, uncertainty, ownership states, and—once version-aware review has occurred—the Profile/procedure pair used to validate those mappings.
...
**Ranking.** `RANKINGS.md` is deterministic. It counts recorded ownership states and never substitutes a numerical maturity model for the categorical evidence.
...
Use the standard documented distribution. Generic extensibility does not earn a positive ownership state merely because custom code could implement a VSM function.

For Methodology `0.3.0`, the standard-distribution boundary may include multiple first-party operating/deployment modes when those modes are actually shipped/documented by the harness. This is how `A(P)` and `C(P)` can record both a base autonomous/constructor mode and a separately evidenced parent-governed mode without claiming simultaneous ownership in one concrete run.

## Function before autonomy / ownership
...
4. identify who owns that right in each first-party mode being claimed: agent, deterministic runtime, developer/configuration, human, parent system, or an explicitly described distributed arrangement;
...
7. only then assign `A`, `A(P)`, `C`, `C(P)`, `P`, `—`, or `?`.

Methodology `0.3.0` applies `A(P)`, `C(P)`, and standalone `P` only to S3, S4, and S5:

- `A(P)` = the autonomous `A` mode is established and a distinct first-party parent-governed mode for the same function is also operationally closed;
- `C(P)` = the constructor `C` mode is established and a distinct first-party parent-governed mode for the same function is also operationally closed;
- `P` = a parent-governed mode is operationally closed, but no first-party `A` or `C` autonomous mode is established at the reviewed boundary.

Composite notation is multi-mode capability notation, not simultaneous dual ownership. A concrete deployment/run still has one reconstructable decisive owner for the relevant organizational decision at a time.

Decision authority is not enforcement authority. A runtime can meter, queue, serialize, block, kill, persist, or validate an already-selected constraint without owning the organizational choice behind it. Hard enforcement is evidence that a decision is operationally enforceable; it is not by itself evidence of agent or parent ownership.
...
Likewise, self-hosting, a stop button, merge review, operator UI, approval hook, maintainer presence, or OSS governance does not by itself justify `(P)`. The parent mode must close the already-established S3/S4/S5 function at the declared recursion.
...
Ordered synthesis considers the currently admitted assessments in ascending `catalog_position`. After a newly admitted `included` assessment is complete, compare it with all earlier admitted assessments and record the smallest informative evidence-backed architectural distinction in `data/signatures.psv`. Identical ownership vectors are valid. Never alter a state to manufacture signature uniqueness.
...
`profile_version` identifies the normative VSM Harness Profile used to establish the organizational mapping. `assessment_procedure_version` identifies the local evidence/ownership procedure used to classify the mapping.
...
Methodology `0.3.0` is such a trigger for S3/S4/S5 ownership classification. Existing canonical assessments do not automatically acquire `(P)` or S3/S4 `P`; they must be explicitly revalidated from primary evidence. Their immutable `generated_*` provenance does not change during that migration.

## Re-review and reassessment

An existing assessment can be revisited in two ways:

- **same-ref correction / semantic revalidation** — the pinned repository revision does not change; stronger evidence, a category-error correction, or a newer assessment semantic changes or confirms the interpretation. The catalog row remains unchanged.
...
A re-review request is evidence-led rather than grade-led. The proposed replacement state is advisory; the reviewer must independently re-establish the VSM function, decisive decision/feedback right, owner, supporting enforcement, closure path, standard-distribution boundary, supported first-party modes, and counter-evidence.
...
- S3 requires current-whole authority over relevant resource/commitment decisions, not merely task decomposition, result aggregation, or deterministic enforcement of a preselected constraint; an S3 parent mode additionally requires legitimate parent ownership and returned current-control closure;
...
- S4 requires external-and-prospective distinctions, adaptation options, and coupling back to present capability; an S4 parent mode additionally requires parent ownership of the decisive adaptation judgment and return into present capability;
- S5 requires legitimate ultimate authority or an operational identity/policy closure path; an S5 parent mode must reach legitimate parent authority and return to govern subsequent operation, not merely static policy text or ordinary task approval.

For `A(P)` or `C(P)`, the reviewer must independently reconstruct both encoded modes. Evidence for the parent mode cannot substitute for evidence that the base `A` or `C` condition exists, and vice versa.
...
A newer upstream commit does **not** by itself require rewriting the assessment. First determine whether changes since the accepted `review_ref` materially affect the organizational behavior represented by the current assessment. Likewise, a newer Profile or Methodology version does not automatically imply a changed vector; it requires explicit revalidation under the newer contract.
...
- **`reassessed-changed`** — the accepted reassessment changes one or more material assessment claims, ownership states, or review-boundary facts;
...
A round is complete only when every harness in its frozen scope has an explicit outcome. Completion does not imply that every assessment changed; a healthy round may consist mostly of `no-upstream-change`, `no-material-change`, or `reassessed-unchanged` results.
...
Therefore `last_checked_ref` may legitimately be newer than `review_ref`. Likewise, `assessment_procedure_version` may advance while `review_ref` remains unchanged when a same-boundary semantic recheck confirms that the assessment still holds under a newer Methodology.
...
For ranking, composite states are parsed by base ownership:

```text
A(P) → base A, parent mode present
C(P) → base C, parent mode present
P    → no A/C base, parent mode present
```

The ranking reports:

- total agent-owned functions: count of base `A` across S1, S2, S3, S3*, S4, S5, so both `A` and `A(P)` count;
- metasystem agent ownership: count of base `A` across S2, S3, S3*, S4, S5;
- constructor count: base `C`, so both `C` and `C(P)` count;
- parent-mode count: `A(P)`, `C(P)`, or standalone `P`;
- count of `?`;
- the exact categorical vector.

Rank by metasystem `A`, then total `A`. Harnesses with equal `(metasystem A, total A)` receive the same rank. Within the same rank, public presentation is newest-first by GitHub repository creation time. Freshness and spec version are presentation/provenance only and never change the rank key. Constructor count, parent-mode presence, and `?` are descriptive only: evidence completeness or governance mode must not become an autonomy tie-breaker. No fractional weights are assigned. `A(P)` does not rank above `A`, and `C(P)` does not rank above `C`.
...
Positive `A`, `A(P)`, `C`, `C(P)`, or `P` claims require primary evidence sufficient to reconstruct every ownership mode encoded by the symbol. `?` means evidence is insufficient. `—` is narrower: no material first-party path is supplied within the reviewed standard-distribution boundary; it is not a universal impossibility claim.
