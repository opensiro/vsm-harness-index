...
last_checked_ref: 027ecf0a379bcc1d09956d46d12d44a3ad9cee14
...
- `python/docs/src/user-guide/agentchat-user-guide/magentic-one.md`: optional user approval is for code execution performed by the operational executor, not for the orchestrator's whole-team plan/progress right.
...
`A`: `MagenticOneOrchestrator` owns a whole-team current facts/plan/progress ledger and has authority to assess progress, choose commitments, reset participants and regenerate the plan when the team stalls. Code-execution approval constrains an operational executor action rather than independently owning this whole-team current-control loop, so no separate `(P)` mode is published. Confidence: high.
