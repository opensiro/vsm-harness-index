...
last_checked_ref: 1cd06c5a2058a172eebadf5d9d7c3fa45c519520
...
- `python/samples/03-workflows/orchestrations/magentic_human_plan_review.py` at `1cd06c5a2058a172eebadf5d9d7c3fa45c519520`: `enable_plan_review=True` requests human approval or revision before executing each generated plan; the review request exposes the current progress ledger and proposed plan; `approve()` continues while `revise(feedback)` returns human feedback into the workflow; responses are supplied back until execution completes.
- The same sample remains present at checked upstream `c030fa3582b1d2971488645fa03ec65bffab8617` with the same blob, confirming that the parent-mode evidence was already available at the accepted review boundary rather than introduced by later upstream changes.
- `dotnet/src/Microsoft.Agents.AI.Workflows/MagenticWorkflowBuilder.cs`: the first-party builder documents plan signoff as review and approve/revise of plans before execution, separately from participant tool-call approval.
