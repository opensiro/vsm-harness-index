...
- External distinction: relevant external documentation, similar skills, best practices, available tool/model behavior and user-environment requirements can expose capability gaps or better approaches that are not contained in the current skill alone.
- Future / prospective distinction: the loop asks whether a reusable skill will handle representative future tasks and trigger appropriately, comparing candidate capability against baselines/previous versions and held-out or expanded evaluations rather than merely repairing the current turn.
- Adaptation option generated: a new or revised reusable `SKILL.md` plus supporting scripts/resources, evaluation design and triggering description selected by the agent from those distinctions and trial results.
- Path back into current capability / S3: the selected skill is written or packaged into Flock-readable user/project/workspace/install skill locations; `flock-skills` subsequently discovers/loads it into later agent contexts, changing the capability available to present operational runs.
