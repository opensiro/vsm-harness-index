...
ASSESSMENTS = {
    ...
    "kot": (ROOT / "assessments" / "kot.md", "7cf18c4b8389a81ea2f15ab70d1b0bb31b44b6ab"),
    "agentharness-alexandrmotologa": (
        ROOT / "assessments" / "agentharness-alexandrmotologa.md",
        "725ceee14b65bd95365e8e1c2394ac1b245e3b98",
    ),
    "rsih": (ROOT / "assessments" / "rsih.md", "737bf1f5a5a56f0c49bbd5180e71d651113038ac"),
    "scion-gcp": (
...
FUNCTIONS = {
    "S2": {
        ...
        "states": {"agentharness-oklahomawhore": "A", "kot": "A", "scion-gcp": "C"},
        "delta": EXP / "s2-system-benchmarks" / "post-closure-deltas" / "late-canonical-admissions-2026-09-26.json",
    ...
    "S3": {
        ...
        "states": {"agentharness-oklahomawhore": "A", "kot": "A", "scion-gcp": "A"},
        "delta": EXP / "s3-system-benchmarks" / "post-closure-deltas" / "late-canonical-admissions-2026-09-26.json",
    ...
    "S3*": {
        ...
        "states": {"kot": "A", "agentharness-alexandrmotologa": "C"},
        "delta": EXP / "s3star-system-benchmarks" / "post-closure-deltas" / "late-canonical-admissions-2026-09-26.json",
    ...
    "S4": {
        ...
        "states": {"rsih": "C", "scion-gcp": "A"},
        "delta": EXP / "s4-system-benchmarks" / "post-closure-deltas" / "late-canonical-admissions-2026-09-26.json",
    ...
    "S5": {
        ...
        "states": {"rsih": "C"},
        "delta": EXP / "s5-system-benchmarks" / "post-closure-deltas" / "late-canonical-admissions-2026-09-26.json",
...
class LateCanonicalCapabilityDeltas735Test(unittest.TestCase):
    def test_public_evidence_deltas_remain_fail_closed(self):
        assessment_fields = {}
        ...
        for function, cfg in FUNCTIONS.items():
            with self.subTest(function=function):
                ...
                for harness_id, state in expected_states.items():
                    ...
                    self.assertTrue(row.get("reassess_if"))
                ...
                for registry_path in cfg["registries"]:
                    ...
                    self.assertFalse(
                        ...
                        f"late-admission systems unexpectedly appeared in {registry_path}",
                    )
