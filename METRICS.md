...
| Canonical assessment records | 171 | Included plus canonical `excluded-no-agentic-vsm` assessment records. |
| Canonical exclusions | 4 | Completed assessments with `status: excluded-no-agentic-vsm`. |
| Proposed intake assessments | 3 | Assessment files still in `status: proposed`; not counted in the canonical corpus. |
| Catalog entries | 171 | Rows in `data/catalog.psv`; this is discovery/order/provenance infrastructure, not a second assessment database. |
| Catalog entries without an included assessment | 4 | `catalog entries - included assessments`; this includes canonical exclusions and is not automatically equivalent to pending work. |
